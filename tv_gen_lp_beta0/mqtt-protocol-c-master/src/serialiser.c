...
mqtt_serialiser_rc_t mqtt_serialiser_write(mqtt_serialiser_t* serialiser, mqtt_message_t* message, uint16_t* buffer, size_t len)
{//mqtt_serialiser_rc_t mqtt_serialiser_write(mqtt_serialiser_t* serialiser, mqtt_message_t* message, uint8_t* buffer, size_t len) { //AS_TEST
  ...
  switch (message->common.type) {
    ...
    {   offset = 1;
        buffer[offset++] = len - 2;
        //WRITE_STRING(message->publish.length);
        //buffer[offset++] = message->publish.topic_name.length;
        WRITE_STRING(message->publish.topic_name);
        //WRITE_STRING(message->publish.content);
        //memcpy(&(buffer[offset]), (message->publish.content.data), (message->publish.content.length));
        for(i = 0; i < message->publish.content.length; i++)
        {
            buffer[offset++] = message->publish.content.data[i];
        }
    ...
    {
        offset = 1;
        buffer[offset++] = message->common.length;
        buffer[offset++] = message->subscribe.message_id / 0xff;
        buffer[offset++] = message->subscribe.message_id & 0xff;

        WRITE_STRING(message->subscribe.topics->name);
        buffer[offset++] = message->subscribe.qos;
    }
    break;
    default: {
