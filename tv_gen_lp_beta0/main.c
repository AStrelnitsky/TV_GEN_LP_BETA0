...
int main(void)
{
    ...
    //    }
      ...
      ///  initWiFi_station();
        ...
        memcpy((uint16_t *)(&readBuf[0]), (uint16_t *)(PROG_ADDR) , sizeof(readBuf));
   ...
   // rect_temp = (uint16_t)data_rx[6];
    if(mqtt.swicth == 1)
    {
            ...
            else
            {
                ...
                if(gen.F < 141000.0) // was 141000.0
                {
                    gen.F = 141000.0;
                }
                //if(gen.F > 1469999.0)
...
void gen_initial_conditions(void)
{
   ...
   // memset(data_tx,0,64);//char data_tx[64], data_rx[64], data_rx_service[64], data_rx_mqtt[64];
    ...
    f_base = 143200.0;//145000.0;//143200.0;
    i_h = 10;
    i_l = 0;
    mqtt.duty = 0;//
    amp_fix = 1.0;
...
__interrupt void sci_isr(void)
{
    ...
    if (stringSeeker(s1, s_got, 32 ,3, &n))
    {
        esp8266.c_status = ESP8266_CWJAP_TERMINAL_OK;
    }
    ...
    else if (stringSeeker(s1, s_4, 10, 3, &n))
    {
        IPD_parser(s1, 4);
    }
                    ...
                    {   if(MMQT_SUBSCRIBE_OK == (mqtt.state_machine))
                        {
                           switch(var_counter)
                           {
                            ...
                            mqtt_message_constructor(&message.publish.content.data[0], &remote_data[10*var_counter], 10, &mqtt_var[0]);
...
void mqtt_parser(char * data)
{
   ...
   // const char * s1_p;
    ...
    const char *str_h_on = "h_on";
    const char *str_h_off = "h_off";
        ...
        case 14:
                                                                                                                   {
                                                                                                                       if(stringSeeker(s1_p, str_af, 55, 4, &kptr))
                                                                                                                          {
                                                                                                                                 mqtt.af = (data_rx_mqtt[kptr + 4] - 48)*1.0 + (data_rx_mqtt[kptr + 6] - 48)*0.1 + (data_rx_mqtt[kptr + 7] - 48)*0.01;
...
void flash_processing(void)
{
    ...
    if(flash_tx)
    {
        ...
        Flash_Program((Uint16 *) (PROG_ADDR), (Uint16 *)progBuf, 64, &FlashStatus);
